...
# --- 🏛️ CITADEL V10.2: TITAN (RECOVERY) ---
WAR_CHEST = [
...
def get_bal():
    ...
    try:
        r = requests.get(f"https://api.mexc.com/api/v3/account?timestamp={ts}&signature={sig}", 
                         headers={"X-MEXC-APIKEY": API_KEY}, timeout=5).json()
...
def fire(coin, side, qty):
    ...
    requests.post(f"https://api.mexc.com/api/v3/order?{params}&signature={sig}", 
...
while True:
    try:
        ...
        os.system('clear')
        print(f"🏛️ TITAN V10.2 | 💰 USDT: ${usdt:.2f} | 💎 XRP: {xrp:.2f}/500")
